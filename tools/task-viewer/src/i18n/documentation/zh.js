...
- **点击复制文档路径**：现在，当您点击任务并进入任务详情页面时，如果列出了任务修改或创建的相关文档，文档名将带有指向文档系统中实际文档的超链接（前提是您在创建/编辑配置文档选项卡时配置了项目文档夹）
...
**解决方案**：完全删除文档上传。现在您必须直接输入文档夹路径，系统会自动附加 \`/tasks.json\`。这确保查看器始终从您的实际实时文档中读取。

**如何使用**：
1. 在终端中导航到您的虾数据文档夹
2. 输入 \`pwd\` 获取完整路径（在 UI 中以黄色突出显示）
3. 将此路径粘贴到"任务文档夹路径"字段中
...
3. **添加您的第一个配置文档**：
   ...
   - 输入包含 tasks.json 的虾数据文档夹的路径
   - **提示：**在终端中导航到您的文档夹并输入 \`pwd\` 以获取完整路径
   - 点击"**添加配置文档**"
